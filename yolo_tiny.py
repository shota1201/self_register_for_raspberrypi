...
class YOLO(object):
    _defaults = {
        "model_path": './trained_weights_final_ver4.h5',
...
def shutter():
    ...
    with picamera.PiCamera() as camera:
        ...
        camera.start_preview(fullscreen=False, window=(100, 100, 960, 1280))
        # sleep(1.000)
        key = input('Please tap [s] and [Enter] if you want to take a picture. If you finish, tap[Enter]: ')
        if key == 's':
...
def detect_img(yolo):
    ...
    try:
        ...
        # r_image, pred_class = yolo.detect_image(image)
    except:
